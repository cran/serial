...
#'    \item{\code{PARITY}}{\emph{n, o, e, m, s} corresponds to 'none', 'odd', 'even', 'mark' and 'space'}
...
#'        }       
#'              
#' @param buffering '\code{none}', best for RS232 serial interface. Connection buffer
#'                  is flushed (send) when ever a write operation takes place.
#'                  '\code{line}', buffer is send after newline character 
#'                  (\\n or 0x0A) is recognized. '\code{full}' write operations will
#'                  be bufferd until a \code{flush(con)} is invoked.
...
serialConnection<-function(name = '', port='com1', mode='115200,n,8,1', buffering='none', newline=0,eof='',translation='auto',handshake='none',buffersize = 4096)
{
         ...
         obj$name <- name
         obj$port <- port
          obj$var <- gsub("[.-]","_",obj$port) # subsitute all '.' by _ to avoid 
                                               # TCL/TK name problems
         obj$mode <- mode
